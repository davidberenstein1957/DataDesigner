...
class EmbeddingColumnConfig(SingleColumnConfig):
    """Configuration for generating embedding vector columns using Large Language Models.

    Embedding columns generate high-dimensional vector representations of text inputs,
    which are useful for semantic search, similarity comparisons, clustering, and other
    vector-based operations. The embeddings are generated by embedding models and stored
    as arrays in the dataset.
    ...
    Attributes:
        ...
        input_type: For asymmetric embedding models, specify "query" or "passage".
            ...
            NVIDIA's llama-3.2-nv-embedqa-1b-v2. Default: "passage".
    ...
    Example:
        ...
        # First create some text columns
        config_builder.add_column(
            SamplerColumnConfig(
                name="product_name",
                sampler_type=SamplerType.CATEGORY,
                params=CategorySamplerParams(values=["Widget", "Gadget", "Gizmo"]),
            )
        )
    ...
    @property
    def required_columns(self) -> list[str]:
        """Get columns referenced in the input_text template.

        Returns:
            List of unique column names referenced in Jinja2 template.
        """
        return list(set(get_prompt_template_keywords(self.input_text)))

    @model_validator(mode="after")
    def assert_input_text_valid_jinja(self) -> Self:
        """Validate that input_text is a valid Jinja2 template.
