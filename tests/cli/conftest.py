...
@pytest.fixture
def stub_inference_parameters() -> ChatCompletionInferenceParameters:
    return ChatCompletionInferenceParameters(temperature=0.7, top_p=0.9, max_tokens=2048, max_parallel_requests=4)


@pytest.fixture
def stub_model_configs(stub_inference_parameters: ChatCompletionInferenceParameters) -> list[ModelConfig]:
    return [
        ModelConfig(
            alias="test-alias-1",
            model="test-model-1",
